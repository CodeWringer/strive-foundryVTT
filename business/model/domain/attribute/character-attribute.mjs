...
export default class CharacterAttribute {
   ...
   */
  get type() {
    const attributeName = ((this._actor.system.attributes[this.name] ?? {}).type ?? business.model.const.ATTRIBUTE_TYPES.SECONDARY.name);
    return business.model.const.ATTRIBUTE_TYPES.asArray().find(it => it.name === attributeName);
  }
  set type(value) {
    this._actor.update({
      system: {
