...
export default class TokenActionPoints extends TokenExtender {
   ...
   */
  addTo(token) {
    if (!common.util.validation.isDefined(token.actor)) return;
   ...
   */
  removeFrom(token) {
    if (common.util.validation.isDefined(token.actionPointContainer)) {
  ...
  async updateOn(token) {
    ...
    if (token.inCombat === true) {
      if (common.util.validation.isDefined(token.actionPointContainer) === true 
        && common.util.validation.isDefined(token.actionPointContainer.text)
        && common.util.validation.isDefined(token.actionPointContainer.text.text)
      ) {
    ...
    } else if (common.util.validation.isDefined(token.actionPointContainer) === true) {
      this.removeFrom(token);
    }
  }
}
