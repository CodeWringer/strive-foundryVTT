...
export default class TokenHealthConditions extends TokenExtender {
  ...
  /** @override */
  async hoverOn(token) {
    if (token.actor.type === business.model.const.ACTOR_TYPES.PLAIN) return;
  ...
  _removeStaticFrom(token) {
    if (common.util.validation.isDefined(token.healthConditionContainer)) {
  ...
  async _addStaticTo(token) {
    if (common.util.validation.isDefined(token.healthConditionContainer)) return;
    if (common.util.validation.isDefined(token.healthConditionHoverContainer)) return;
    ...
    for (let i = 0; i < Math.min(this.MAX_STATIC_ICONS + 1, healthConditions.length); i++) {
      ...
      if (common.util.validation.isDefined(healthCondition.img)) {
        const texture = await PixiLoader.load(healthCondition.img);
        if (!common.util.validation.isString(texture)) {
    ...
    if (entryCount === this.MAX_STATIC_ICONS && healthConditions.length > this.MAX_STATIC_ICONS) {
      const localized = common.util.string.format2(game.i18n.localize("system.character.health.condition.tokenMore"), {
   ...
   */
  async _removeHover() {
    if (common.util.validation.isDefined(game.canvas.tokens.striveHoverLayer)) {
  ...
  async _addHoverTo(token) {
    ...
    if (common.util.validation.isDefined(striveHoverLayer.healthConditionHoverContainer)) return;
    ...
    for await (const healthCondition of healthConditions) {
      ...
      container.position.set(x, y);
      let xInContainer = 0;
      if (!common.util.validation.isDefined(striveHoverLayer.healthConditionHoverContainer)) {
        game.strive.logger.logWarn("Undefined container reference");
        continue;
      }
