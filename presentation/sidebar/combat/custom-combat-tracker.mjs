...
export default class CustomCombatTracker extends FoundryWrapper.CombatTracker {
  ...
  get enableMomentumBar() {
    return GameSystemWorldSettings.get(GameSystemWorldSettings.KEY_ENABLE_MOMENTUM_BAR);
  }
  ...
  async getData(options) {
    ...
    for (const turn of data.turns) {
      ...
      if (ValidationUtil.isDefined(document) !== true) {
        ...
        continue;
      } else if (document.type === business.model.const.ACTOR_TYPES.PLAIN) {
        continue;
      } 
      
      // Add action points view model. 
      turn.renderActionPoints = document.type !== business.model.const.ACTOR_TYPES.PLAIN;
      ...
      turn.renderGritPoints = transientActor.type === business.model.const.ACTOR_TYPES.PC 
        || (transientActor.type === business.model.const.ACTOR_TYPES.NPC && transientActor.gritPoints.enable === true);
